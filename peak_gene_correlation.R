#to get average expression/peak level by cell type
PeakByCellType <- sapply(split(colnames(peak_matrix_sr@assays$RNA@data), peak_matrix_sr$major_type), #colnames(peak_matrix_sr@assays$RNA@data): expression matrix, colnames; peak_matrix_sr$major_type: meta.data
                         function(cells) rowMeans(peak_matrix_sr@assays$RNA@data[,cells])) #peak_matrix_sr@assays$RNA@data: seurat matrix;
#write it to csv
